  ...
  validation {
    condition = alltrue([
      for rule in var.lifecycle_rules : (
        ...
        (lookup(try(rule.expiration, {}), "date", null) == null || can(formatdate("YYYY-MM-DD", rule.expiration.date))) &&
        ...
        alltrue([
          ...
          (lookup(transition_rule, "date", null) == null || can(formatdate("YYYY-MM-DD", transition_rule.date)))
        ])
      )
    ])
    error_message = "Expiration or transition date must be in YYYY-MM-DD format (e.g., '2026-01-13')."
  }
}